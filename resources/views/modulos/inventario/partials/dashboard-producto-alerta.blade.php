<div class="grid gap-3 p-4 sm:grid-cols-[1fr_auto]">
    <div class="min-w-0">
        ...
        <p class="mt-1 text-xs text-muted-foreground">
            {{ $producto->sku ?? 'Sin SKU' }} &middot; {{ $producto->categoria?->nombre ?? 'Sin categoria' }} &middot; {{ $producto->proveedor?->nombre ?? 'Sin proveedor' }}
        </p>
        <p class="mt-1 text-xs text-muted-foreground">
            Stock {{ $producto->formatearCantidadConUnidad($producto->cantidadStock()) }}
            &middot; Alerta {{ $producto->formatearCantidadConUnidad($producto->cantidad_alerta_stock) }}
        </p>
    </div>

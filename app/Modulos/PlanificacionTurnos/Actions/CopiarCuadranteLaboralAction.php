...
/**
 * Replica una semana completa desplazando cada jornada al mismo día relativo.
 */
class CopiarCuadranteLaboralAction
{
    public function __construct(private readonly CrearJornadaLaboralAction $crearJornada) {}

    public function ejecutar(CuadranteLaboral $origen, Carbon $semanaDestino): CuadranteLaboral
    {
        ...
        return DB::transaction(function () use ($lunes, $origen): CuadranteLaboral {
            $destino = CuadranteLaboral::query()->create([
                'semana_inicio' => $lunes,
                'estado' => EstadoCuadranteLaboral::Borrador,
                'notas' => 'Copiado de la semana '.$origen->semana_inicio->format('d/m/Y').'.',
            ]);

            foreach ($origen->jornadas()->get() as $jornada) {
                $desplazamiento = $origen->semana_inicio->diffInDays($jornada->fecha);

                $this->crearJornada->ejecutar($destino, [
                    'usuario_id' => $jornada->usuario_id,
                    'area_trabajo_id' => $jornada->area_trabajo_id,
                    'fecha' => $lunes->copy()->addDays($desplazamiento)->toDateString(),
